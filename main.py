...
def llm_surf(llm_input: dict) -> str:
    ...
    if os.getenv("MODE") == "multi-agent":
        MA = Multi_Agent(ChatWithOpenai=ChatWithOpenai, client=azure_connector_surf.client)
    ...
    # running in single-agent mode
        ...
        results_string, results_json, function_name = SA.generate_answer(llm_input=llm_input, content=content)
